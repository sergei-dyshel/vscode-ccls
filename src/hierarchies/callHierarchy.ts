import { CallHierarchyIncomingCall, CallHierarchyItem, CallHierarchyOutgoingCall,
  ...
  commands, Position, SymbolKind, TextDocument, TreeItem, Uri, workspace } from 'vscode';
...
class CclsCallHierarchyItem extends CallHierarchyItem {
  ...
  constructor(node: CallHierarchyNode, p2c: Converter) {
    ...
    const uri = Uri.parse(node.location.uri);
    super(
        SymbolKind.Function, node.name, workspace.asRelativePath(uri), uri,
        range, range);
    this.node = node;
  }
}
